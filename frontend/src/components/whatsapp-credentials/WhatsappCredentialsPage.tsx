...
// ==============================================
// CONTROLE TEMPORÁRIO DE INTEGRAÇÃO META
// ==============================================
// Esta variável controla o estado da integração com a Meta
// enquanto não temos as permissões necessárias aprovadas.
//
...
export function WhatsappCredentialsPage() {
  ...
  // Estados para modal de ativação do número
  const [showActivationModal, setShowActivationModal] = useState(false);
  const [selectedCredentialForActivation, setSelectedCredentialForActivation] = useState<EvolutionCredential | null>(null);
  const [activationPin, setActivationPin] = useState('');
  const [activatingNumber, setActivatingNumber] = useState(false);
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        if (adsCredentials.length > 0) {
          ...
          // Verificar credenciais com status 'pending' para ativação automática
          const pendingCredentials = adsCredentials.filter((cred: any) => cred.status === 'pending');
          if (pendingCredentials.length > 0) {
            console.log(`🔄 Encontradas ${pendingCredentials.length} credenciais ads com status 'pending'`);
            
            // Aguardar um pouco para não sobrecarregar a API
            setTimeout(async () => {
              for (const credential of pendingCredentials) {
                try {
                  await handleAutoActivation(credential);
                } catch (error) {
                  console.warn(`⚠️ Erro na ativação automática da credencial ${credential.id}:`, error);
                }
              }
            }, 2000); // Aguardar 2 segundos antes de tentar ativação automática
          }
  ...
  useEffect(() => {
    if (credentials.length > 0) {
      const pendingCredentials = credentials.filter(
        (cred: any) => cred.connection_type === 'ads' && cred.status === 'pending'
      );
      
      if (pendingCredentials.length > 0) {
        console.log(`🔄 Encontradas ${pendingCredentials.length} credenciais com status 'pending' para ativação automática`);
        ...
        }, 3000); // Aguardar 3 segundos antes de tentar ativação automática
  ...
  // Função para ativar número do WhatsApp
  const activateWhatsAppNumber = async (credentialId, pin) => {
    try {
      setLoading(true);
      const response = await fetch(`/api/whatsapp-credentials/activate-number/${credentialId}`, {
        method: 'POST',
        headers: {
      ...
      if (data.success) {
        ...
        loadCredentials(); // Recarregar lista
      } else {
        showError(data.message);
      }
    } catch (error) {
      console.error('Erro ao ativar número:', error);
      showError('Erro ao ativar número do WhatsApp');
    } finally {
      setLoading(false);
    }
  };

  // Função para ativação automática quando status for pending
  const handleAutoActivation = async (credential) => {
    if (credential.connection_type === 'ads' && credential.status === 'pending') {
      // Verificar se já tentou ativação automática recentemente
      const lastAutoActivation = credential.metadata?.last_auto_activation;
      const now = new Date().getTime();
      const oneHour = 60 * 60 * 1000; // 1 hora em ms

      if (!lastAutoActivation || (now - new Date(lastAutoActivation).getTime()) > oneHour) {
        console.log(`🔄 Tentando ativação automática para credencial ${credential.id}`);
        
        // Gerar PIN automático (pode ser configurável)
        const autoPIN = '123456'; // Em produção, isso deve vir de configuração
        
        try {
          await activateWhatsAppNumber(credential.id, autoPIN);
        } catch (error) {
  ...
  // Função para ativação manual com PIN
  const handleActivationClick = (credential) => {
    setSelectedCredentialForActivation(credential);
    setActivationPin('');
  ...
  const executeActivation = async () => {
    if (!selectedCredentialForActivation || !activationPin.trim()) {
      showError('PIN é obrigatório para ativação');
      return;
    }

    try {
      setActivatingNumber(true);
      await activateWhatsAppNumber(selectedCredentialForActivation.id, activationPin.trim());
      setShowActivationModal(false);
      setSelectedCredentialForActivation(null);
      setActivationPin('');
    } catch (error) {
      console.error('Erro na ativação:', error);
    } finally {
      setActivatingNumber(false);
    }
  };
      ...
      <div className="p-3 sm:p-6 bg-gradient-to-br from-emerald-950 via-cyan-950 to-blue-950 min-h-screen">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
          ...
          <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-3 w-full sm:w-auto">
            ...
            <button
              onClick={META_INTEGRATION_ENABLED ? handleOpenMetaSignup : undefined}
              disabled={!META_INTEGRATION_ENABLED}
              className={`px-3 sm:px-4 py-2 rounded-lg transition-colors flex items-center justify-center space-x-2 text-sm sm:text-base ${
                META_INTEGRATION_ENABLED 
                  ...
                  : 'bg-gray-600/20 text-gray-400 cursor-not-allowed opacity-60'
              }`}
              title={META_INTEGRATION_ENABLED ? 'Conectar conta Meta/WhatsApp Business' : 'Funcionalidade em desenvolvimento - Em breve disponível!'}
            >
              <FaFacebook className="mr-2" />
                              <span>
                  {META_INTEGRATION_ENABLED ? 'Conectar Meta' : 'Conectar Meta (Em Breve)'}
                </span>
            </button>
            <button
                ...
                <div className="p-3">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    >
                      ...
                      {/* Botão Ativar Número - apenas para credenciais 'ads' com status 'pending' */}
                      {credential.connection_type === 'ads' && credential.status === 'pending' && (
                        <button
                          onClick={() => handleActivationClick(credential)}
                          className="px-2 py-1.5 rounded-md bg-purple-500/20 text-purple-300 text-xs hover:bg-purple-500/30 transition-colors flex items-center justify-center"
                          title="Ativar número do WhatsApp com PIN"
                        >
        ...
        {/* Seção de Templates de Mensagens */}
        {credentials.some(cred => cred.connection_type === 'ads' && cred.wpp_business_account_id) && (
          <div className="mt-8">
            <WhatsappTemplatesSection 
              businessAccountId={credentials.find(cred => cred.connection_type === 'ads')?.wpp_business_account_id}
              accessToken={credentials.find(cred => cred.connection_type === 'ads')?.wpp_access_token}
            />
          </div>
        )}
                ...
                >
                  <Dialog.Panel className="w-full max-w-lg transform overflow-hidden rounded-lg bg-gradient-to-b from-cyan-900 to-slate-900 p-6 text-left align-middle shadow-xl transition-all border border-cyan-700/50">
                    ...
                    <div className="grid grid-cols-1 gap-4">
                      ...
                      {/* Opção Anúncios - Visível mas desabilitada durante desenvolvimento */}
                      <button
                        onClick={META_INTEGRATION_ENABLED ? () => handleSelectConnectionType('ads') : undefined}
                        disabled={!META_INTEGRATION_ENABLED}
                        className={`group relative p-4 rounded-lg border transition-all duration-300 text-left ${
                          META_INTEGRATION_ENABLED
                            ? 'border-cyan-700/40 hover:border-cyan-500 bg-gradient-to-r from-slate-800/20 to-slate-700/20 hover:from-slate-700/30 hover:to-slate-600/30'
                            : 'border-slate-700/60 bg-gradient-to-r from-slate-900/30 to-slate-800/30 cursor-not-allowed opacity-60'
                        }`}
                        title={META_INTEGRATION_ENABLED ? 'Conectar via API oficial da Meta para anúncios' : 'Funcionalidade em desenvolvimento - Em breve disponível!'}
                      >
                        <div className="flex items-start justify-between mb-2">
                          <div className="flex items-center space-x-2">
                            <div className={`p-2 rounded-md ${
                              META_INTEGRATION_ENABLED ? 'bg-cyan-600/15 text-cyan-300' : 'bg-slate-700/30 text-slate-400'
                            }`}>
                              <FaAd className="text-lg" />
                            </div>
                            <div>
                              <h3 className={`font-semibold text-base ${
                                META_INTEGRATION_ENABLED ? 'text-white' : 'text-slate-400'
                              }`}>
                                API Oficial para Anúncios
                              </h3>
                              <p className={`text-xs ${
                                META_INTEGRATION_ENABLED ? 'text-gray-300' : 'text-slate-500'
                              }`}>
                                {META_INTEGRATION_ENABLED 
                                  ? 'Integração direta com a API oficial da Meta para gerenciamento de anúncios.'
                          ...
                          {META_INTEGRATION_ENABLED ? (
                            ...
                            <>
                              <div className="flex items-center space-x-2 text-xs text-slate-500">
                                ...
                                <span>Integração com API oficial da Meta</span>
                              </div>
                              <div className="flex items-center space-x-2 text-xs text-slate-500">
                                <FaClock className="text-cyan-400 text-[10px]" />
                                <span>Gerenciamento de campanhas publicitárias</span>
                              </div>
                              <div className="flex items-center space-x-2 text-xs text-slate-500">
                                <FaClock className="text-cyan-400 text-[10px]" />
...
{/* Mensagem informativa removida conforme solicitado */}
        ...
        {/* Modal de Ativação do Número */}
        <Transition appear show={showActivationModal} as={Fragment}>
          <Dialog as="div" className="relative z-50" onClose={() => setShowActivationModal(false)}>
            <Transition.Child
              as={Fragment}
              enter="ease-out duration-300"
              enterFrom="opacity-0"
            ...
            <div className="fixed inset-0 overflow-y-auto">
              <div className="flex min-h-full items-center justify-center p-4 text-center">
                <Transition.Child
                  ...
                  leaveTo="opacity-0 scale-95"
                >
                  <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-lg bg-gradient-to-b from-emerald-900 to-slate-900 p-6 text-left align-middle shadow-xl transition-all border border-emerald-700/50">
                    <Dialog.Title
                      as="h3"
                      className="text-lg font-semibold leading-6 text-emerald-100 mb-4 flex items-center"
                    >
                    ...
                    <div className="mb-6">
                      <p className="text-gray-300 text-sm mb-4">
                        Digite o PIN de ativação recebido no WhatsApp para ativar o número.
                      </p>
                      
                      <div className="mb-4">
                        <label htmlFor="activationPin" className="block text-sm font-medium text-gray-300 mb-2">
                          PIN de Ativação
                        </label>
                        <input
                          type="text"
                          ...
                          className="w-full px-3 py-2 bg-slate-800 border border-slate-600 rounded-md text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent"
                          maxLength={6}
                        />
                      </div>

                      {selectedCredentialForActivation && (
                        <div className="bg-slate-800/50 border border-slate-600 rounded-lg p-3 mb-4">
                          <div className="text-sm text-gray-300">
                            <p><strong>Número:</strong> {selectedCredentialForActivation.phone}</p>
                            <p><strong>Tipo:</strong> {selectedCredentialForActivation.connection_type === 'ads' ? 'API Oficial' : 'WhatsApp Business'}</p>
                          </div>
                        </div>
                      )}
                    </div>
                    ...
                    <div className="flex justify-end space-x-3">
                      ...
                      <button
                        ...
                        disabled={activatingNumber || !activationPin.trim()}
                        className="px-4 py-2 rounded-md bg-emerald-600 text-white hover:bg-emerald-700 transition-colors flex items-center disabled:opacity-50 disabled:cursor-not-allowed"
                      >
                        {activatingNumber ? (
                          <>
