...
function getDefaultDownloadsFolder() {
    ...
    const downloadsPath = path_1.default.join(homeDir, "Downloads", "mcp-screenshots");
    ...
    constructor(app, server) {
        ...
        this.app.post("/screenshot", (req, res) => {
            ...
            try {
                ...
                const fullPath = path_1.default.join(targetPath, filename);
                ...
                const messageHandler = (message) => {
                    try {
                        ...
                        if (response.type === "screenshot-data" &&
                            ...
                            const dir = path_1.default.dirname(response.path);
                            fs_1.default.mkdirSync(dir, { recursive: true });
                            // Generate a unique filename using timestamp
                            const timestamp = new Date().toISOString().replace(/[:.]/g, "-");
                            const filename = `screenshot-${timestamp}.png`;
                            const fullPath = path_1.default.join(response.path, filename);
                            // Write the file
                            fs_1.default.writeFileSync(fullPath, base64Data, "base64");
                            resolve({
                                path: fullPath,
